...
Процедура ИтераторСоздается() Экспорт
	...
	// Когда
	Результат = Новый ИтераторЗначениеСоответствие(Новый Соответствие, ЭтотОбъект);

	// Тогда

	Ожидаем.Что(Результат).ИмеетТип("ИтераторЗначениеСоответствие");
...
Процедура ЕстьСледующий() Экспорт
	...
	Итератор = Новый ИтераторЗначениеСоответствие(Коллекция, ЭтотОбъект);
...
Процедура НетСледующего() Экспорт
	...
	Итератор = Новый ИтераторЗначениеСоответствие(Новый Соответствие, ЭтотОбъект);
...
Процедура СледующийВызываетИсключениеПриКонкурентнойМодификации() Экспорт
	...
	Итератор = Новый ИтераторЗначениеСоответствие(Соответствие, ЭтотОбъект);
...
Процедура ДляКаждогоОставшегося() Экспорт
	...
	Коллекция = Новый Соответствие;
	Коллекция.Вставить(0, 1);
	Коллекция.Вставить(1, 2);
	Коллекция.Вставить(2, 3);
...
Процедура ДляКаждогоОставшегосяЛямбда() Экспорт
	...
	Ожидаем.Что(ТестМодуль.СуммаЭлементов).Равно(5);
	Ожидаем.Что(Итератор.ЕстьСледующий()).ЭтоЛожь();
...
Процедура ДляКаждогоОставшегосяЛямбдаКонтекстСтруктура() Экспорт
	...
	Ожидаем.Что(Контекст.СуммаЭлементов).Равно(5);
	Ожидаем.Что(Итератор.ЕстьСледующий()).ЭтоЛожь();
...
Процедура ПриМутацииИтераторомИнвалидируютсяДругиеИтераторы() Экспорт
	...
	ИтераторМутирующий       = Новый ИтераторЗначениеСоответствие(Коллекция, ЭтотОбъект);
	ИтераторИнвалидирующийся = Новый ИтераторЗначениеСоответствие(Коллекция, ЭтотОбъект);
