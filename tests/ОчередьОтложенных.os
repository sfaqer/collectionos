...
&Тест
Процедура ЭлементыУпорядоченыПоВозрастаниюЗадержки() Экспорт
	...
	ОчередьОтложенных.Положить(Новый Отложенный(Сейчас + 30000, 3, Ложь));
	ОчередьОтложенных.Положить(Новый Отложенный(Сейчас + 1000, 1, Ложь));
	ОчередьОтложенных.Положить(Новый Отложенный(Сейчас + 10000, 2, Ложь));

	// Тогда

	Результат = ОчередьОтложенных.ВМассив();

	Ожидаем.Что(Результат).ИмеетДлину(3);
	Ожидаем.Что(Результат[0].Значение()).Равно(1);
	Ожидаем.Что(Результат[1].Значение()).Равно(2);
	Ожидаем.Что(Результат[2].Значение()).Равно(3);

КонецПроцедуры

&Тест
Процедура ВзятьВозвращаетГотовыйЭлементНезависимоОтОстальныхЗадержек() Экспорт
	...
	ОчередьОтложенных.Положить(Новый Отложенный(Сейчас + 30000, 2, Ложь));
	ОчередьОтложенных.Положить(Новый Отложенный(Сейчас - 5000, 1, Ложь));

	// Когда

	Подсмотренный = ОчередьОтложенных.Подсмотреть();
	Результат     = ОчередьОтложенных.Взять();

	// Тогда

	Ожидаем.Что(Подсмотренный.СодержитЗначение()).Равно(Истина);
	Ожидаем.Что(Подсмотренный.Получить().Значение()).Равно(1);

	Ожидаем.Что(Результат.СодержитЗначение()).Равно(Истина);
	Ожидаем.Что(Результат.Получить().Значение()).Равно(1);

	Ожидаем.Что(ОчередьОтложенных.Количество()).Равно(1);

КонецПроцедуры

&Тест
Процедура ВзятьНеВозвращаетЭлементСНеистекшейЗадержкой() Экспорт
	...
	ОчередьОтложенных.Положить(Новый Отложенный(Сейчас + 30000, 1, Ложь));
	...
	Ожидаем.Что(Подсмотренный).Равно(Опциональные.Пустой());
	Ожидаем.Что(Результат).Равно(Опциональные.Пустой());
