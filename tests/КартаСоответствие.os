...
Процедура ДляКаждогоЛямбдаКонтекстСтруктура() Экспорт
	...
	ПроверяемаяКарта.ДляКаждого(
		"(Ключ, Значение) -> Контекст.ДляКаждогоВыполнилсяРаз = Контекст.ДляКаждогоВыполнилсяРаз + 1;
		|	Контекст.СуммаЭлементов = Контекст.СуммаЭлементов + Ключ + Значение;",
	...
	Ожидаем.Что(Контекст.СуммаЭлементов).Равно(12);
...
Процедура ДляКаждогоЛямбдаКонтекстОбъект() Экспорт
	...
	ПроверяемаяКарта.ДляКаждого(
		"(Ключ, Значение) -> ДляКаждогоВыполнилсяРаз = ДляКаждогоВыполнилсяРаз + 1;
		|	СуммаЭлементов = СуммаЭлементов + Ключ + Значение;",
		ТестМодуль
	);
...
&Тест
Процедура ВычислитьБезусловноЛямбдаКонтекстСтруктура() Экспорт
	...
	Результат = ПроверяемаяКарта.ВычислитьБезусловно("1", "Ключ, Значение -> Результат", Новый Структура("Результат", 2));
...
&Тест
Процедура ВычислитьБезусловноЛямбдаКонтекстОбъект() Экспорт
	...
	Результат = ПроверяемаяКарта.ВычислитьБезусловно("1", "Ключ, Значение -> СуммаЭлементов", ТестМодуль);
...
&Тест
Процедура ВычислитьЕслиОтсутствуетЛямбдаКонтекстСтруктура() Экспорт
	...
	Результат = ПроверяемаяКарта.ВычислитьЕслиОтсутствует("4", "Ключ -> Результат", Новый Структура("Результат", 4));
...
&Тест
Процедура ВычислитьЕслиОтсутствуетЛямбдаКонтекстОбъект() Экспорт

	// Дано

	ТестМодуль.СуммаЭлементов = 4;
	...
	Результат = ПроверяемаяКарта.ВычислитьЕслиОтсутствует("4", "Ключ -> СуммаЭлементов", ТестМодуль);
...
&Тест
Процедура ВычислитьЕслиПрисутствуетЛямбдаКонтекстСтруктура() Экспорт
	...
	Результат = ПроверяемаяКарта.ВычислитьЕслиПрисутствует(
		"1",
		"Ключ, Значение -> Результат",
		Новый Структура("Результат", 2)
	);
...
&Тест
Процедура ВычислитьЕслиПрисутствуетЛямбдаКонтекстОбъект() Экспорт
	...
	Результат = ПроверяемаяКарта.ВычислитьЕслиПрисутствует("1", "Ключ, Значение -> СуммаЭлементов", ТестМодуль);
...
&Тест
Процедура СлитьЛямбдаКонтекстСтруктура() Экспорт
	...
	Результат = ПроверяемаяКарта.Слить(
		"1",
		2,
		"СтароеЗначение, НовоеЗначение -> СтароеЗначение + НовоеЗначение + Значение",
		Новый Структура("Значение", 3)
	);
	...
	Ожидаем.Что(Результат).ИмеетТип("Опциональный");
	Ожидаем.Что(Результат.Пустой()).ЭтоЛожь();
	Ожидаем.Что(Результат.Получить()).Равно(6);

	Ожидаем.Что(Соответствие).ИмеетДлину(3);
	Ожидаем.Что(Соответствие.Получить("1")).Равно(6);
...
&Тест
Процедура СлитьЛямбдаКонтекстОбъект() Экспорт

	// Дано

	ТестМодуль.СуммаЭлементов = 3;
	...
	Результат = ПроверяемаяКарта.Слить(
		"1",
		2,
		"СтароеЗначение, НовоеЗначение -> СтароеЗначение + НовоеЗначение + СуммаЭлементов",
		ТестМодуль
	);
...
Процедура ЗаменитьВсеЛямбдаКонтекстСтруктура() Экспорт
	...
	ПроверяемаяКарта.ЗаменитьВсе("Ключ, Значение -> Число(Ключ) + Значение + Соль", Новый Структура("Соль", 1));
	...
	Ожидаем.Что(Соответствие.Получить("1")).Равно(3);
	Ожидаем.Что(Соответствие.Получить("2")).Равно(5);
	Ожидаем.Что(Соответствие.Получить("3")).Равно(7);
...
Процедура ЗаменитьВсеЛямбдаКонтекстОбъект() Экспорт
	...
	ПроверяемаяКарта.ЗаменитьВсе("Ключ, Значение -> Число(Ключ) + Значение + СуммаЭлементов", ТестМодуль);
...
Процедура ПреставлениеПереопределяется() Экспорт
	...
	Соответствие = Соответствие();
	Соответствие.Вставить(4, ПроверяемаяКарта);
	Соответствие.Вставить(5, ТестМодуль.ОбъектЗаглушка());
	...
	Ожидаем.Что(Результат)
		.Равно("{1=1, 2=2, 3=3, 4=(Эта карта), 5=Заглушка}");
