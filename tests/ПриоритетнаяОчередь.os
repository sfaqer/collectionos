...
&Тест
Процедура ПриоритетнаяОчередьСоздаетсяКомпараторЛямбдаКонтекстСтруктура() Экспорт

	// Дано

	Контекст = Новый Структура("Делитель", 2);
	...
	Результат = Новый ПриоритетнаяОчередь("Первый, Второй -> {
		...
		|		Возврат 1;
		|	ИначеЕсли Первый % Делитель = Второй % Делитель Тогда
		|		Возврат 0;
		|	Иначе
		|		Возврат -1;
		|	КонецЕсли;
...
&Тест
Процедура ПриоритетнаяОчередьСоздаетсяКомпараторЛямбдаКонтекстОбъект() Экспорт
	...
	Результат = Новый ПриоритетнаяОчередь("Первый, Второй -> {
		...
		|		Возврат 1;
		|	ИначеЕсли Первый % СуммаЭлементов = Второй % СуммаЭлементов Тогда
		|		Возврат 0;
		|	Иначе
		|		Возврат -1;
		|	КонецЕсли;
