...
Процедура ПреставлениеПереопределяется() Экспорт
	...
	Результат = Строка(ФиксированнаяКоллекция);
...
Процедура ОбходитсяЦикломДляКаждого() Экспорт
	...
	Для каждого Элемент Из ФиксированнаяКоллекция Цикл
...
Функция КоллекцияФикстура()
	...
	МетодИтератор = Новый Метод("Итератор")
		.Публичный()
		.ТелоМетода("Возврат Новый ИтераторЗначениеСоответствие(ВнутренняяКарта.Соответствие, ЭтотОбъект);");
